...
class Counter {
	 ...
	 * @param {number} [tsRefreshMs=null] optional timestamp refresh interval, minimal 10s
	 * @memberof Counter
	 */
	constructor(name, help, labels, tsRefreshMs = null) {
		let config;
		this.hashMap = {};
		this.tsRefreshMs = !!tsRefreshMs && tsRefreshMs < 10e3 ? 10e3 : tsRefreshMs; // if set,  min. 10 seconds
		...
		if (Number.isInteger(this.tsRefreshMs)) {
			setInterval(() => {
				const maxOldTs = Date.now() - tsRefreshMs;

				Object.keys(this.hashMap).forEach(key => {
					const hash = this.hashMap[key];
					// eslint-disable-next-line no-param-reassign
					if (!!hash.timestamp && hash.timestamp <= maxOldTs)
						this.hashMap[key].timestamp = Date.now();
				});
			}, this.tsRefreshMs).unref();
		}
	}
